...
static CONSOLE_CURSOR_INFO    outCursorInfo;
static COORD    outCursorPos;
static CPINFOEXW cpinfoex = {0, };
static HWND hWndMyself = NULL;
...
static OPTVAL win32val[2];
#define o_codepage    win32val[0].value.number
#define o_wheeldelta  win32val[1].value.number
static int setcp(OPTDESC *opt, OPTVAL *val, CHAR *newval)
{
    ...
    o_codepage = newcp;
    GetCPInfoExW(o_codepage, 0, &cpinfoex);
    return 1;
}
...
static OPTDESC win32desc[] =
{
    ...
    {"wheeldelta", "whd",  optnstring,    optisnumber}
};
...
DWORD IMEGetMode() {
    ...
    int mode1 = SendMessage(hWndIME, WM_IME_CONTROL, 0x0001, 0);
    int mode2 = SendMessage(hWndIME, WM_IME_CONTROL, 0x0005, 0);
    return (mode2 ? 0x80000000 : 0x00000000) | mode1;
#else
    if (hWndMyself == NULL) {
        DWORD pid = GetCurrentProcessId();
        EnumWindows(MyEnumWindowProc, (LPARAM)pid);
    }
    HWND hWndIME = ImmGetDefaultIMEWnd(hWndMyself);
    int mode1 = SendMessage(hWndIME, WM_IME_CONTROL, 0x0001, 0);
    int mode2 = SendMessage(hWndIME, WM_IME_CONTROL, 0x0005, 0);
    return (mode2 ? 0x80000000 : 0x00000000) | mode1;
#endif
...
static void IMEOff() {
    ...
    HWND hWndIME = ImmGetDefaultIMEWnd(hWndMyself);
    int bImeOn = SendMessage(hWndIME, WM_IME_CONTROL, 0x0005, 0);
    if (bImeOn) {
        SendMessage(hWndIME, WM_IME_CONTROL, 0x0006, 0);
    }
}

int isleadbyte(unsigned char ch) {
    if (cpinfoex.MaxCharSize <= 1) return 0;
    for (int i = 0; i < MAX_LEADBYTES / 2; i++) {
        if (ch >= cpinfoex.LeadByte[2 * i] && ch <= cpinfoex.LeadByte[2 * i + 1]) return 1;
        if (cpinfoex.LeadByte[2 * i] == 0 && cpinfoex.LeadByte[2 * i + 1] == 0) return 0;
    }
    return 0;
}
...
void ttyinit()
{
    ...
    GetConsoleCursorInfo(outConsole, &outCursorInfo);
    GetConsoleScreenBufferInfo(outConsole, &csbi);
    outCursorPos = csbi.dwCursorPosition;
...
void ttynormal()
{
    if (useconsole)
    {
        ...
        SetConsoleCursorInfo(outConsole, &outCursorInfo);
        SetConsoleCursorPosition(outConsole, outCursorPos);
    }
    else
    {
        /* switch to text mode */
...
{
    INPUT_RECORD event, events[10];/* buffer, holds an input record */
...
int i;
    ...
    do
    {
        ...
        // after Windows 8.1, ReadConsoleInput has been changed in multibyte environment, see the following
        // https://blogs.msdn.microsoft.com/japan_platform_sdkwindows_sdk_support_team_blog/2016/05/20/readconsoleinput-%E9%96%A2%E6%95%B0%E3%82%92%E5%91%BC%E3%81%B3%E5%87%BA%E3%81%99%E3%81%A8%E6%AC%A1%E3%81%AE%E6%96%87%E5%AD%97%E3%81%AE%E4%B8%8A%E4%BD%8D1%E3%83%90%E3%82%A4%E3%83%88%E3%81%8C%E8%BF%94/
        if (!ReadConsoleInputW(inConsole, events, 10, &nevents))
        {
        ...
        for (int i = 0; i < nevents; i++) {
            ...
            case KEY_EVENT:
                {
#if 01
                    if (event.Event.KeyEvent.bKeyDown && event.Event.KeyEvent.uChar.UnicodeChar == L'\x1b') {
                        //DWORD imemode = IMEGetMode();
                        //DWORD a = imemode;
                        IMEOff();
                    }
                    for (x = event.Event.KeyEvent.wRepeatCount;
                        ...
                        x--)
                    {
                        if (event.Event.KeyEvent.bKeyDown && event.Event.KeyEvent.wVirtualKeyCode == 0 && event.Event.KeyEvent.wVirtualScanCode == 0 && event.Event.KeyEvent.uChar.UnicodeChar != 0) {
                            // this key event is from IME
                            CHAR tmpbuff[10];
                            UINT l = WideCharToMultiByte(932, 0, &event.Event.KeyEvent.uChar.UnicodeChar, 1, tmpbuff, 10, NULL, NULL);
                            for (int i = 0; i < l; i++) {
                                buf[got++] = tmpbuff[i];
                            }
                        ...
                        else if (event.Event.KeyEvent.uChar.AsciiChar && event.Event.KeyEvent.bKeyDown)
                        {
                            /* plain text character*/
                            buf[got++] = event.Event.KeyEvent.uChar.AsciiChar;
                        }
                        else if (3 == (char)event.Event.KeyEvent.wVirtualScanCode && event.Event.KeyEvent.bKeyDown)
                        {
                            /* Ctrl-2 should be a NUL character */
                            buf[got++] = '\0';
                        }
                        else if (7 == (char)event.Event.KeyEvent.wVirtualScanCode && event.Event.KeyEvent.bKeyDown)
                        {
                            /* Ctrl-6 should be a ^^ character */
                            buf[got++] = ELVCTRL('^');
                        }
                        else if (event.Event.KeyEvent.wVirtualScanCode >= ' ' && event.Event.KeyEvent.bKeyDown
...
#else
                if (event.Event.KeyEvent.uChar.AsciiChar != 0 && event.Event.KeyEvent.bKeyDown) {
                    buf[got++] = event.Event.KeyEvent.uChar.AsciiChar;
                    return got;
                }
#endif
            ...
            case MOUSE_EVENT:
                ...
                if (event.Event.MouseEvent.dwEventFlags & MOUSE_WHEELED) {
                    if (event.Event.MouseEvent.dwButtonState & 0x80000000) {
                        eventscroll(gw, SCROLL_FWDLN, o_wheeldelta, 0);
                        return -2;
                    } else {
                        eventscroll(gw, SCROLL_BACKLN, o_wheeldelta, 0);
                        return -2;
                    }
                }
 ...
 static ELVBOOL       soset = ElvFalse;/* is standout set? */
    ...
    for (i = 0; i < len; i++)
    {
        ...
        else if (console == outConsole && argno == -1)
        {
            ...
            SetConsoleCursorPosition(console, coord);
            WriteConsoleA(console, &buf[i], j, &dummy, NULL);
        ...
        else if (buf[i] == '\n')
        {
            ...
            else
            {
                ...
                ci.Char.AsciiChar = ' ';
                ci.Attributes = attr;
                ScrollConsoleScreenBufferA(console, &rect, NULL, dest, &ci);
            }
        }
        else if (buf[i] == '\r')
        {
        ...
        else if (argno < 0)
        {
            ...
            for (j = 1; i + j < len && ((unsigned char *)buf)[i + j] >= ' '; j++)
            {
            }
            ...
            if (console == myConsole)
                FillConsoleOutputAttribute(myConsole, myattr,
                    j, coord, &dummy);
            WriteConsoleOutputCharacterA(console, &buf[i],
                j, coord, &dummy);
#endif
        ...
        else
        {
            ...
            switch (buf[i])
            {
              ...
              case 'J':
                ...
                if (qty > 0)
                {
                    FillConsoleOutputCharacterA(console, ' ', qty, coord, &dummy);
...
char *ttytermtype()
{
    ...
    useconsole = !strcmp(type, "console") || !strcmp(type, "cygwin") || !strcmp(type, "pcbios");
    if (useconsole)
    {
        inConsole = GetStdHandle(STD_INPUT_HANDLE);
